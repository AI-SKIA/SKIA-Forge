...
import {
    ...
    performLogin,
    showLoginOverlay,
} from "./skia/skiaAuthPanel";
import { t } from "./i18n/forgeI18n";
...
const syncSettingsAuthButton = (): void => {
    const authBtn = document.getElementById("settings-logout-btn") as HTMLButtonElement | null;
    const inlineAuth = document.getElementById("settings-auth-inline");
    const connected = hasActiveSession();
    if (inlineAuth) {
        inlineAuth.hidden = connected;
    }
    if (!authBtn) return;
    const signedOutFlash = authBtn.dataset.flash === "signed-out";
    if (signedOutFlash) return;
    if (connected) {
        authBtn.hidden = false;
        authBtn.textContent = t("settings.signOut");
    } else {
        authBtn.textContent = t("settings.signIn");
        authBtn.hidden = Boolean(inlineAuth);
    }
};

const initializeSettingsInlineAuth = (): void => {
    const emailInput = document.getElementById("settings-auth-email") as HTMLInputElement | null;
    const passInput = document.getElementById("settings-auth-password") as HTMLInputElement | null;
    const rememberBox = document.getElementById("settings-auth-remember") as HTMLInputElement | null;
    const submitBtn = document.getElementById("settings-auth-submit") as HTMLButtonElement | null;
    ...
    void window.skiaElectron.getSavedCredentials()
        .then((saved) => {
            ...
            if (!passInput.value) passInput.value = saved.password;
            if (rememberBox) rememberBox.checked = true;
        })
        .catch(() => undefined);

    const runInlineLogin = async (): Promise<void> => {
        ...
        try {
            ...
            submitBtn.textContent = t("settings.signIn");
        }
    };
...
const initializeSettingsControlsOnce = (): void => {
    ...
    logoutBtn?.addEventListener("click", () => {
        if (hasActiveSession()) {
            logout();
            logoutBtn.dataset.flash = "signed-out";
            logoutBtn.textContent = t("settings.signedOut");
            window.setTimeout(() => {
                delete logoutBtn.dataset.flash;
                syncSettingsAuthButton();
            }, 1500);
            return;
        }
        showLoginOverlay();
        const inlineAuth = document.getElementById("settings-auth-inline");
        if (inlineAuth && !inlineAuth.hidden) {
            (document.getElementById("settings-auth-email") as HTMLInputElement | null)?.focus();
        }
    });
...
const initializeSidebarNavigation = (): void => {
    ...
    document.getElementById("explorer-tree")?.addEventListener("click", (event) => {
        ...
        if (target.id === "explorer-open-folder-hint") {
